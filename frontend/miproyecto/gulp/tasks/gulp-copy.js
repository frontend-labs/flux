...
function Task(gulp, path, options, plugins, settings) {
    ...
    gulp.task('copy:js:libs', function () {
        return gulp.src(path.copy.js.libs.src, { base: path.copy.js.libs.base })
    ...
    gulp.task('copy:fonts', function () {
        return gulp.src(path.copy.fonts.src, { base : path.src.static.fonts })
            .pipe(gulp.dest(path.copy.fonts.dest));
    });

    gulp.task('copy:img', function () {
        return gulp.src(path.copy.images.source.src, { base : path.src.static.images })
    ...
    gulp.task('copy:img:sprites', function () {
        return gulp.src(path.copy.images.sprites.src, { base : path.src.static.images })
            .pipe(plugins.if(settings.config.prod, plugins.imagemin(options.imagemin.general)))
